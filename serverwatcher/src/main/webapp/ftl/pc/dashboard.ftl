            ...
            var jqxhr = $.get("dashboard/healthMonitor", { id: 0, cacheId: cacheId },
                    function (data) {
                        if (data != undefined) {
                            ...
                            var content = '<tr><th>Component</th>';
                            content += '<th>URL</th>';
                            content += '<th>Last Up Time</th>';
...
//                              content += '<td class="" rowspan='+serverApp.serverComponents.length+'>'+teamContacts+'</td>';
                                $.each(serverApp.componentGroups, function (index, group) {
                                    ...
                                    $.each(group.componentList, function (index, component) {
                                        ...
                                        content += '<td class="warning"><h5><small>'+component.lastUpTime+'</small></h5></td>';
                                        content += '<td class="warning"><h5><p><small>'+component.response+'</small></p></h5></td>';
